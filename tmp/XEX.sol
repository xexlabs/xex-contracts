...
contract XEX is ERC20, Ownable, IXEX {
    ...
    error TermTooShort();
    error TermTooLong(uint term, uint maxTerm);
     ...
     */
    function getMaxTerm() public view returns (uint) {
    ...
    function getMinTerm() public view returns (uint) {
        return MIN_TERM + 1;
    }
    /**
    ...
    function claimRank(uint term) external {
        ...
        if (termSec >= getMaxTerm() + 1) revert TermTooLong(termSec / 1 days, getMaxTerm() / 1 days);
     ...
     */
    function claimMintReward(address to) public returns (uint rewardAmount) {
        ...
        rewardAmount = _calculateMintReward(mintInfo.rank, mintInfo.term, mintInfo.maturityTs, mintInfo.amplifier, mintInfo.eaaRate) * 1 ether;
    ...
    function claimMintRewardAndStake(uint pct, uint term) external {
        ...
        if (term * 1 days > MAX_TERM_END + 1) revert TermTooLong(term, MAX_TERM_END / 1 days);
