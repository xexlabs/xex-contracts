...
task('zeropad', 'zeropad')
  ...
  .setAction(async (taskArgs) => {
    ...
    const result = ethers.utils.hexlify(
      ethers.utils.zeroPad(taskArgs.value, 32),
    );
    console.log(result);
  });
...
task('create-private-key', 'Create a new private key').setAction(async () => {
  const { ethers } = require('hardhat');
...
task('halt', 'stop minting').setAction(async (taskArgs) => {
  const lzFile = 'lz-mainnet.json';
  const contractsFile = 'contracts-mainnet.json';
  const lz = JSON.parse(fs.readFileSync(lzFile));
  const contracts = JSON.parse(fs.readFileSync(contractsFile));
...
task('setTreasure', 'set treasure address')
  ...
  .setAction(async (taskArgs) => {
    ...
    if (!contract) {
      return console.log(
        `Invalid chain ${network.chainId} (${contractsFile}).`,
      );
    }
...
task('claim', 'execute a claim').setAction(async (taskArgs) => {
  const lzFile = 'lz-testnet.json';
  const contractsFile = 'contracts-testnet.json';
  const lz = JSON.parse(fs.readFileSync(lzFile));
  const contracts = JSON.parse(fs.readFileSync(contractsFile));
  ...
  const proof = [
    '0x21f179225765b68879783a2211bbf63ca4115c86ca7411dac88086f68f087da0',
    '0x4b9693dd7615d825eb90a35798bb959dd375e847f822809f2dd126869d045b6b',
...
task('retry', 'retry a stuck payload in the bridge')
  ...
  .setAction(async (taskArgs) => {
    ...
    const tx = await main.retryPayload(
      taskArgs.id,
      taskArgs.contract,
      taskArgs.payload,
    );
...
module.exports = {
  networks: {
    ...
    arb: {
      url: `https://rpc.ankr.com/arbitrum/${process.env.ANKR}`,
      accounts: [`${process.env.PRIVATE_KEY}`],
      live: true,
      saveDeployments: true,
    },
