    ...
    try {
      if (cfg.chain !== 'localhost') {
        const initialMint = new BigNumber(cfg.initialMint)
          ...
          .toFixed();
        console.log(
          `verify main: ${res.main}(${cfg.fee}, ${cfg.endpoint}, ${initialMint})`,
        );
...
async function deploy(cfg) {
  ...
  await math.deployed();
  if (cfg.chain !== 'localhost') await math.deployTransaction.wait(10);
  ...
  await main.deployed();
  if (cfg.chain !== 'localhost') await main.deployTransaction.wait(10);
  ...
  if (cfg.chain !== 'localhost') await factory.deployTransaction.wait(10);
